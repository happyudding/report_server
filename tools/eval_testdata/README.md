...
> **Compare 모드용은 별도 스크립트 3개다** — 검출 정본이 eval_engine signature 룰이 아니라
> `web_report/tabs/compare.py _dist_focus` 라 축이 완전히 다르다.
> - [make_compare_testdata.py](make_compare_testdata.py) (v1, 2026-08-20) — **현행 임계가
>   기대대로 도는가** 회귀 검증. 레벨 **L1(미검출)/L2(검출)/L3(강검출)** 3단 → §6
> - [make_compare_shape_testdata.py](make_compare_shape_testdata.py) (v2/v3, 2026-08-28) —
>   **임계 자체를 다시 잡기 위한** 데이터. 현행 지표의 미검출(모양만 다른 경우)과
>   과검출(개선인데 잡히는 경우)을 동시에 드러낸다. 레벨 **L1~L4 = 눈으로 본 차이 크기**
>   (임계 초과 정도가 아니다) → §7. **이 역할은 끝났다** — v3 룰 개정이 이 데이터로 이뤄졌다.
> - [make_compare_level_testdata.py](make_compare_level_testdata.py) (**v4**, 2026-09-01) —
>   **레벨이 곧 검출 여부**인 회귀 데이터. L1·L2 미검출 / L3·L4 검출 → §8
...
## 8. Compare **레벨 정렬** 데이터 (`make_compare_level_testdata.py`, v4 / 2026-09-01)

```
server\.venv\Scripts\python.exe tools\eval_testdata\make_compare_level_testdata.py
```

**§7(v2/v3) 과 목적이 다르다.** v2/v3 는 레벨과 검출이 **독립**이었고, 그 어긋남
(blind_spot·over_detect)이 곧 산출물이라 임계를 다시 잡는 재료였다. 그 역할은 끝났다 —
현행 v3 룰(`_dist_focus` 7단계)이 그 데이터로 만들어졌다.

v4 는 반대로 **레벨이 곧 검출 여부**다:

| 레벨 | 기대 |
|---|---|
| L1 | **미검출** (여유 있게 임계 아래) |
| L2 | **미검출** (임계 직하 — 룰이 느슨해지면 여기가 먼저 걸린다) |
| L3 | **검출** (임계 초과) |
| L4 | **검출** (크게 초과) |

용도는 회귀 검증이다 — Issue Table Compare 에서 item 명의 레벨로 정렬하면
**L1·L2 구간에 검출이 하나도 없고 L3·L4 구간이 전부 검출**이어야 한다.

### 8-1. 산출물 4개 (`data/`)

| 파일 | 내용 |
|---|---|
| `compare_level_v4_before.csv` | 7-meta honeyform (Before) — 1009 die × 136 item |
| `compare_level_v4_after.csv` | 7-meta honeyform (After) |
| `compare_level_v4_answer.csv` | 정답표 — item·유형·레벨·기대 검출·검출 경로·역산된 강도 t |
| `compare_level_v4_verify.csv` | **서버 코드 실측** 대조 (`ok=0` 이 불일치) |

### 8-2. 핵심 장치 — 목표 지표 **역산**(`solve_t`)

유형마다 "강도"의 의미가 제각각이라(봉우리 간격·꼬리 비율·격자 폭 …) 손으로 상수를 고르면
유형마다 검출 경계가 어긋난다. v3 실측이 그랬다 — 같은 L3 인데 `POS_SEPARATE` 는 ks_d
1.00, `SHAPE_TAIL_ONESIDE` 는 0.056 이었다(전자는 L1 부터 검출, 후자는 L4 까지 미검출).

v4 는 모양 강도를 **연속 파라미터 t** 로 두고 레벨별 목표 지표에 닿도록 **이분탐색으로
역산**한다. 목표는 현행 판정축 위에 잡는다:

```
ks 경로   L1 0.05 / L2 0.10 / L3 0.24 / L4 0.42   (임계 0.15)
Cpk 경로  L1 1.58 / L2 1.50 / L3 1.15 / L4 0.80   (임계 1.33)
σ 경로    L1 5%  / L2 16%  / L3 34%  / L4 70%     (임계 20%)
```

검출 경로를 유형마다 갈라 둔 이유는, 전부 ks 로 맞추면 ④(절대 품질)·⑤(산포) 경로의
회귀를 이 데이터로 볼 수 없기 때문이다.

### 8-3. 만들면서 확인된 것 3가지 (고칠 때 반드시 알아야 한다)

1. **단순 클립은 σ 를 갉아먹는다** → `_fit_moments_in_spec`.
   `_rescale` 로 σ 를 맞춰도 뒤의 spec 클립이 극단값을 잘라 σ 가 도로 줄어든다.
   꼬리·왜곡·스파이크는 손실이 커서(실측 −20~−50%) After **Cpk 가 오르고**, 그러면
   `_dist_focus` ③ 이 "개선"으로 보고 제외한다 — 모양이 완전히 달라진 L3·L4 가 통째로
   미검출이 됐다. 클립↔재스케일을 번갈아 돌려 두 조건을 동시에 만족시킨다.
2. **판정 경로는 서로 겹친다.** `CPK_LOW` 의 L2 목표를 Cpk 1.42 로 잡았더니 ④ 는 피했는데
   ⑥(형태)에 걸렸다 — Cpk 를 떨어뜨리는 수단이 평균 이동이라 1.42 는 이미 0.55σ 이동이고
   그때 ks 가 0.21 로 임계를 넘는다. 한 경로만 보고 목표를 잡으면 다른 경로가 잡아챈다.
   (목표를 1.50 으로 올려 해결)
3. **비단조 유형이 있다** → `solve_t` 의 격자 스캔 대체.
   꼬리 방향 반전(`SHAPE_TAILFLIP`)의 ks 는 t=1 에서 0.25 로 정점을 찍고 t=2 에서 0.08 로
   **되돌아온다** — 양쪽 꼬리가 함께 길어지면 두 거울상 분포가 다시 닮아가기 때문이다.
   이분탐색만 쓰면 상한에 잘못 수렴한다(실측: 목표 0.24 → 0.11, L3·L4 미검출).

### 8-4. 구조적 상한이 있는 유형

`SHAPE_UNIFORM`(정규→균등)은 **L1·L2 만** 만든다. 두 ECDF 가 중앙에서 교차해 ks 상한이
약 0.06 이라 L3·L4 를 만들 수 없고, 억지로 강도를 올리면 σ 가 달라져 의도한 ks 경로가
아닌 다른 경로로 잡힌다. 같은 이유로 새 유형을 넣을 때 목표에 못 닿으면
`levels=LOW_LV` 로 내리는 것이 정직한 처리다 — verify 가 그 신호를 미검출로 준다.

### 8-5. 실행 결과 (2026-09-01)

```
item 136 · 유형 20 · die 1009
L1 35건 미검출 / L2 35건 미검출 / L3 33건 검출 / L4 33건 검출
전 136개 항목이 레벨 기대와 일치
```

경계 여유 — L2 최대 ks_d **0.126** (임계 0.15 아래), 대조군 `CONTROL_SAME`(before/after
동일 분포)은 전 레벨 미검출이다.
