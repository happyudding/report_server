...
그래서 런처는 **바뀐 파일만 받고, 안 바뀐 것은 현재 버전 폴더에서 하드링크**(안 되면 복사)
한다. 판정 기준은 각 버전 폴더에 들어 있는 `.files.json`(파일별 sha256)이다 — 디스크를
...
삼키고**, `current.txt` 는 설치가 완전히 끝난 뒤에만 바뀐다. 새 버전은 `*.tmp-<pid>` 에
다 만든 뒤 이름만 바꾸므로 반쯤 만들어진 폴더가 정식 버전으로 보이지 않는다.
...
| 8 | 설치 중 런처 강제 종료 | 다음 실행에서 3.1.1 정상, 잔재 회수 |
| 9 | 진행창에서 취소 | 3.1.1 실행, `versions\` 에 `*.tmp-*` 없음 |
...
* **런처 자신(`Honey.exe`)은 자동 업데이트되지 않는다** — 실행 중인 파일이라 덮을 수 없다.
  런처가 바뀌는 릴리스는 재설치 안내가 필요하다(그래서 런처는 거의 안 바뀌게 설계했다).
