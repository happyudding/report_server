...
_PREP_ITEM_LIST_MAX = 30


def build_preprocess_rows(payload):
    """적용 중 전처리 → [[구분, 내용], …]. 적용된 것이 없으면 빈 리스트(시트 생략).

    규칙 문안은 web_report.preprocess.describe_rule 를 그대로 쓴다 — 클라에서 따로
    조립하면 서버 저장값과 표기가 갈린다(그 모듈이 순수 모듈인 이유).
    """
    pp = payload if isinstance(payload, dict) else {}
    spec = pp.get("spec") if isinstance(pp.get("spec"), dict) else {}
    rows = []

    summary = str(pp.get("summary") or "")
    if summary:
        rows.append(["요약", summary])

    excl = spec.get("exclude_items") or []
    if excl:
        names = [str(x) for x in excl]
        shown = names[:_PREP_ITEM_LIST_MAX]
        text = ", ".join(shown)
        if len(names) > len(shown):
            text += f" 외 {len(names) - len(shown)}개"
        rows.append([f"항목 제외 ({len(names)}개)", text])

    outlier = spec.get("outlier")
    if isinstance(outlier, dict) and outlier.get("k") is not None:
        rows.append(["Outlier 제거", f"각 항목 평균 ± {outlier['k']}σ 밖의 값을 제외"])

    edits = spec.get("edits") or []
    if edits:
        rows.append([f"셀 수정 ({len(edits)}건)", "빠른 수정으로 지정한 개별 셀 값 치환"])

    rules = spec.get("rules") or []
    if rules:
        try:
            from web_report.preprocess import describe_rule
        except Exception:
            describe_rule = None
        for idx, rule in enumerate(rules, 1):
            ...
            if describe_rule:
                try:
                    text = describe_rule(rule) or ""
                except Exception:
                    text = ""
            rows.append([f"일괄 규칙 {idx}", text or "(규칙 설명을 만들 수 없음)"])

    basis = pp.get("yield_basis")
    if basis:
        gross = pp.get("gross_die")
        label = {"gross": "Gross Die", "test": "Test data 개수"}.get(str(basis), str(basis))
        text = label + (f" ({gross})" if str(basis) == "gross" and gross else "")
        rows.append(["수율 분모", text])
