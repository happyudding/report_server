...
# docs/23 초안의 500자 상한 → 1000자(5줄) → **1800자**(2026-09-02, 상한이 10줄이 되면서).
# ⚠ 이 상한은 **잘라내기**다 — 모자라면 마지막 줄이 문장 중간에서 끊긴 채 저장된다.
# 줄 수 상한(_INSTRUCTION)을 올릴 때 이 값도 같이 볼 것. 한 줄을 "핵심 단어만" 으로
# 짧게 쓰라고 지시하므로 10줄 × 여유 180자면 실측 분량을 덮는다.
MAX_SUGGESTION_CHARS = 1800
...
_INSTRUCTION = (
    ...
    "[제안] 전체는 최대 10줄이고 한 signature 를 다루는 데 5줄을 넘기지 마라.\n"
...
_INSTRUCTION_EXTRA = (
    ...
    "대조는 머릿속으로만 하고 그 근거 수치와 지표 이름을 문장에 옮겨 적지 마라 - 결과로 무엇을 확인할지만 남겨라."
    "10줄은 상한이지 채워야 하는 목표가 아니다 - 발화 signature 를 전부 덮고 나서도 쓸 근거가 없으면 줄 수를 줄여라."
