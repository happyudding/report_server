# COINAPI report_server — Codex 진입점
...
2. 사용자가 product_type / product / lot_id + 4자리 PIN 입력 + xlsx 선택. 클라이언트가
   **Excel COM 으로 DRM 해제·시트 셀값을 읽어** summary/yield/issue_table grid(2D 배열)와
   issue_table 행별 PNG 를 추출 → `POST /pe/report/upload_xlsx` (xlsx 파일 미전송,
   `sheet_grids` JSON + `issue_img_<row>` 만)
3. 서버: sha256(canonical(sheet_grids) + meta) → analysis_key → DB 세션 생성(PIN 저장) →
   grid 파싱 → yield_rows DB 저장, sheet_data 텍스트 DB 저장, issue PNG S3(로컬 폴백) 보관.
   **원본 xlsx 는 서버에 저장하지 않는다.**
...
(구 `source_xlsx` 는 폐지 — 원본 xlsx 를 보관하지 않는다.)
