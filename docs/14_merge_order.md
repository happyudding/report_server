...
> 구서버(외부 소유) 버전의 `client/report_generator/` 와 `server/storage_gateway/` 를
> 나중에 **폴더째 교체**할 때, 신서버 전용 확장이 유실되지 않도록 하는 **병합 순서·계약
...
`report_generator`·`storage_gateway` 는 외부에서 소유·교체된다. 그런데 신서버는 이 둘에
신서버 전용 기능을 **추가**했다(웨이퍼 bin map, web_report parquet/manifest 저장, S3 헬스/
...
```
1) report_generator 폴더 교체 (구서버 버전)
        │  구서버 df_honey.from_ddi_paths / from_ddi_paths_by_dut 가 딸려옴
        ▼
...
- 이유: 신서버 `client/report_generator/df_honey.py` 에는 `from_csv` 만 있고 구서버가 쓰는
  `from_ddi_paths` / `from_ddi_paths_by_dut` 가 없다. honey_main 의 MDDI 분기가 이 진입점을
  요구하므로, 진입점이 딸려오는 report_generator 교체가 선행되어야 한다.
- **지금 신서버에서 `from_ddi_paths*` 를 새로 구현하지 말 것** (교체로 충족 — A-3).
...
- 신서버가 추가해 **보존 대상**인 함수: `save_webreport_sources` / `save_webreport_manifest` /
...
- **facade 경계**: 프로젝트 코드는 내부 `_s3`·`_issue_images` 를 직접 import 하지 않는다.
  ...
  - 확인: `grep -rn "storage_gateway\._s3\|storage_gateway import _s3\|import _issue_images" server/`
    → 비어야 함.
- **교체본 `_s3.py` 가 보존할 신서버 상수/env**: prefix 3종(`REPORT_S3_DIST_COMBINED_PREFIX`,
...
- 구서버가 쓰는 `from_ddi_paths` / `from_ddi_paths_by_dut` 는 report_generator 교체로
  자연히 딸려온다 → **신서버에서 미리 구현하지 말 것**.
...
- [ ] `report_generator/` 를 구서버 버전으로 교체 (map 코드 없는 상태 유지 확인).
