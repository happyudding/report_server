...
> CSV/xlsx 측정 데이터 → 통계 분석 → **templete.xlsx 기반** Excel 리포트 생성. 반도체 mass_data(웨이퍼/로트).
...
| [df_honey_group.py](../client/report_generator/df_honey_group.py) | **`df_honey_group`**: 다중 묶음 + select/filter (df 슬라이싱 위임) |
...
| [xlsx_writer.py](../client/report_generator/xlsx_writer.py) | **하이브리드**: templete.xlsx 를 openpyxl 로 채움 + distribution 만 xlwings |
...
- **df_honey_group** = `{source_name: df_honey}`. `select_items`(subject 필터)/`filter_rows_by_bin`(Bin1 Only)/`split_by_dut`(단일파일 DUT별 분할 → DUT 가 source/legend) 는 위 슬라이싱 메서드에 **위임**. `subjects()`/`limits` 는 **첫 source 기준**.
...
**Phase 1 (openpyxl)** — 동봉 `templete.xlsx`(`_template_path`, frozen 대응)를 열어 table 시트 **값만** 채움(서식은 템플릿 보유, 가변 행은 셀 `_capture_style`/`_apply_style` 복제, 표영역 병합 해제). 표는 **B열~·헤더 3행**.
- **summary** 3 섹션: `1.Device Feature`(Total DUT/Pass/Fail Types/Sources/Subjects/**EVT Version**), `2.Yield`(Lot NO/Yield/Major Fail 1st~5th = subject+ratio), `3.Evaluation`(Yield/CPK/Temp/ETC).
- **yield** = `bin | Item | {src}_count | {src}_yield | avg | comment` (다중입력시 소스쌍 반복).
- **fail_item** = yield 와 동일.
- **cpk** = `TEST NAME | LOW SPEC | HIGH SPEC | SCALE | 계열 | n … cpk | comment`.
- **issue_table** = `Category`(Yield 블록=yield 재사용, CPK/ETC 플레이스홀더) `| bin | Item | avg | {src}… | Distribution | comment…`.
...
- `templete.xlsx` 는 [build_honey.spec](../client/build_honey.spec) `datas` 로 동봉(`data/templete.xlsx`).
- numpy 스칼라는 `_sanitize_cell`/`_json_safe` 로 python 기본형 변환.
