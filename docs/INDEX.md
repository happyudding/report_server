...
| 23 | **AI Comment `[제안]` 클라이언트 LLM 대행** (⚠ 미구현 — 구현 의뢰서). 서버에 Enterprise 자격증명이 없어 업로더 PC 의 Honey 가 사내 Gateway 호출을 대행하고 서버가 병합. Options 패널 AI Model 콤보(default/claude) 포함 | Client + Server | [23_ai_comment_client_llm.md](23_ai_comment_client_llm.md) | [web_report/ai_comment.py](../web_report/ai_comment.py) · [recommend.py](../eval_analyzer/eval_engine/pipeline/recommend.py) |
