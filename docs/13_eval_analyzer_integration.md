...
- **탭 8개** (표본함은 §14):
  ...
  7. *AI 지시문* (2026-09-02) — AI Comment **[제안]** 문장을 만드는 LLM 에게 주는 추가
     ...
     ⚠ 2026-09-02 재설계 이후 **사례가 0건인 item 은 LLM 을 아예 부르지 않는다** — 지시문에
     "사례가 없을 때" 를 다루는 문장을 넣을 필요가 없다(docs/23 §재설계).
...
>
> ~~⚠ **판정 밴드는 여전히 `tail_mass_3s`(양쪽 합)에 건다.**~~ → **5차 재편에서 뒤집혔다**
> (아래 §16-5 ①). 당시 "부작용" 으로 본 현상(UNKNOWN 겨냥 5건이 BIDIR_TAIL 로 뒤집힘)이
> 사용자가 원한 판정이었다 — 그 5건은 실제로 양쪽 꼬리가 두꺼운 모양이다.
>
> **⑤ `LOW_SAMPLE_UNCERTAIN` 삭제**(꺼진 채였다. 표본 부족은 `n_min` 가드와
> `data_completeness` 가 이미 말한다) · **⑥ `outlier_sigma` 4.5 → 2.5.**
> ⚠ ⑥ 의 경고(“BIMODALITY 게이트에 물려 있다”)는 **현실이 됐다** — 실제로 이봉 항목이
> 통째로 막혔다. 5차 재편에서 게이트 전용 컷을 분리해 해소했다(아래 §16-5 ②).
>
...
### 16-5. 5차 재편 — 꼬리 방향 밴드 · 이봉 게이트 분리 (2026-08-19)

사용자가 v12 회귀 데이터를 눈으로 검토하고 지적한 4건에서 출발했다. **넷 다 실측으로
원인을 확정한 뒤** 고쳤다(v12 CSV 를 `eval_engine.pipeline` 으로 직접 재평가, 115 item 전수).

**① 꼬리 밴드를 방향별 질량에 건다** — 4차의 결정을 뒤집는다.

`USL_TAIL`/`LSL_TAIL` 의 밴드가 `tail_mass_3s`(양쪽 **합**)에 걸려 있어서, 양방향으로 꼬리가
두꺼운 항목이 **둘 다 미발화**했다. 그러면 `BIDIR_TAIL` 의 `replaces` 도 영영 발동하지
못한다 — 정작 BIDIR 로 불러야 할 모양이 UNKNOWN 으로 떨어진다.

| | 4차(양쪽 합) | 5차(방향별) |
|---|---|---|
| 대칭 꼬리 한쪽 3% (합 6%) | 상한 5% 초과 → **둘 다 미발화** | 각 3% 는 밴드 안 → 둘 다 발화 → `BIDIR_TAIL` |
| 단방향 꼬리 (한쪽 2%, 반대쪽 0) | 발화 | 발화(변화 없음) |

실측(UNKNOWN 겨냥 5건): kurtosis 15~16 통과, 방향 균형 0.48/0.52 통과, 그런데 합
0.066~0.091 이 상한 0.05 에 걸려 미발화 → UNKNOWN. 전 115 항목 시뮬레이션에서 이 전환으로
**기존 단방향 겨냥 8건 무변화 · 오탐 0**. 덤으로 `OUTLIER_L5` 에 `BIDIR_TAIL` 이 붙던 오염도
사라졌다(각 방향 0.6~0.7% 로 밴드 하한 미만인데 합치면 밴드에 들어왔었다).
`tail_side_share_*` 조건은 **보조로 남긴다** — 반대쪽에 점 한둘이 섞였을 뿐인 경우를
여전히 한쪽 꼬리로 읽는다.

**② BIMODALITY 게이트 전용 컷 분리** — `subpop_outlier_sigma: 4.5` 신설.

게이트가 표시용 `outlier_sigma` 를 함께 쓰고 있어서, 4차에서 그 값을 4.5 → 2.5 로 내리자
outlier 비율이 0.010 → **0.153** 으로 뛰어 이봉 판정이 조용히 죽었다. **표시 감도와 판정
게이트를 한 손잡이에 묶어 둔 것**이 원인이라 키를 갈랐다. 화면·evidence 의 `outlier_ratio`
는 사용자가 지시한 2.5 그대로다.

**③ 이봉 판정 완화** — 떨어져 나간 소수 무리를 분리로 본다.

| 키 | 전 | 후 | 이유 |
|---|---|---|---|
| `subpop_outlier_ratio_max` | 0.03 | **0.15** | 떨어져 나간 무리는 **그 자체가 outlier 로도 세어진다** — 0.03 은 "떨어진 이봉" 을 구조적으로 전부 막았다(실측 0.109) |
| `subpop_density_gap_strong` | 0.5 | **0.20** | 소수 무리 분리는 히스토그램 골이 얕게 나온다 |
| `subpop_minor_mass_min` | 0.05 | **0.0003** | 사실상 비활성화 |

⚠ **실질 판별자는 `density_gap ≥ 0.20` 하나다.** minor_mass 는 하한이 꺼진 셈이라 되돌릴
때 둘을 함께 봐야 한다. 안전한 근거 — 값 축 구조가 거의 같은 두 항목(빈 구간 0.30 vs
0.35, 소수쪽 질량 0.0034 vs 0.0004)을 density_gap 이 **0.0039 대 0.2279 로 58배** 갈라
준다. 앞은 OUTLIER, 뒤는 분리다. 전수 시뮬레이션에서 겨냥 세트 오염 0건.
`SPECIFICITY_ORDER` 에서 `BIMODALITY` 를 `OUTLIER` **앞**으로 옮겼다 — 둘이 함께 뜰 때
"무리로 갈라졌다" 가 "몇 개가 튀었다" 보다 구체적인 설명이고, 순서가 반대면 화면
Signature 컬럼에 늘 OUTLIER 만 보인다.

**④ OUTLIER 끊김 임계 0.35 → 0.30.** USL 쪽으로 fail 26개가 떨어진 항목이 0.3043 으로
아슬하게 미달해 UNKNOWN 이 됐다. 임계를 낮춘 대신 **꼬리 겨냥 데이터를 더 촘촘하게**
만들어(`tools/eval_testdata` `TAIL_P` 0.03 → 0.06) 두 유형의 간격을 데이터 쪽에서 벌렸다
— 실측 jump 가 0.07~0.34 → 0.04~0.09 로 내려가 임계에서 멀어졌다.

**함께 고친 것 (생성기)**

- **EDGE_TOP/EDGE_BOTTOM 이름이 화면과 반대였다.** 웨이퍼 맵은 y축을 뒤집어 그리므로
  (`static/webreport/wafer_charts.js` `autorange:"reversed"`) **YPOS 가 작을수록 화면 위**인데
  생성기는 `edge_top` 을 +90°(y 큰 쪽)로 잡았다. 축 부호를 뒤집었다.
- **`BIDIR_TAIL` 이 한 번도 검증되지 않고 있었다.** 유일한 겨냥 세트가 `_bidir_plan`
  (양쪽 margin<1σ = 전량 fail)이라 `CSV_EXCLUDE` 로 CSV 에서 빠졌고, `replaces` 경로는
  겨냥 자체가 없었다. **양쪽 꼬리** 세트 `bidir_tails_specs()` 를 신설했다(fail 이 자연
  꼬리뿐이라 CSV 1장에 들어간다).
- **UNKNOWN 겨냥군을 옮겼다.** ① 전환으로 종전 모양이 BIDIR_TAIL 이 되므로, 넓은 성분
  비중을 키워(`w_wide` 0.10~0.16 → 0.20~0.28) **kurtosis 로 막는다**(7 대 10). 질량 밴드
  경계(5%)를 오가던 것보다 여유가 넓다.
- **`natural` 모드 item 에 chip 우선권**을 줬다. 종전엔 미분류군에만 있었는데 꼬리 겨냥도
  같은 성질이다 — 극단 chip 을 앞 item 에 뺏기면 그 값이 몸통 안으로 당겨져(chip 1개는
  FAILTNO 를 하나만 갖는다) 4제곱 지표가 주저앉는다. 실측으로 `USL_TAIL_L2` 가 목표 12.0
  대비 9.96 으로 미발화했다.

**검증**: eval_analyzer 209 passed · `rules_io.validate_all()` problems 0 ·
`perf_guard --scan-all` 위반 0 · `test_rules_io_guards` / `test_issue_signature` 통과 ·
회귀 데이터 `data/eval_testdata_7meta_v13*` **120/120 의도대로**(누락 0 · 오발화 0 ·
정상군 오탐 0 · UNKNOWN 겨냥 5/5). eval.db **스키마 변경 없음**(방향별 질량은 v10 컬럼).
⚠ `THRESHOLD_RELATIONS` 에서 `subpop_density_gap_warn <= subpop_density_gap_strong` 을
뺐다 — 두 값은 강약 서열이 아니라 **서로 다른 판정의 컷**이다(이봉이냐 / 분리냐).
