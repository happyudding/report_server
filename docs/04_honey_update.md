...
- **UAC 승격 (2026-08-26 신설, 런처 레이아웃 한정)** — 구 batch 경로는 여전히 승격을
  쓰지 않지만(위 줄), 런처는 **로컬 권한/잠금으로 막혔을 때만** UAC 를 한 번 묻는다.
  - 판정은 `app_update.prepare_target()` 이 던지는 `LocalWriteError` 하나다. 네트워크
    실패와 반드시 구분한다 — 권한 실패에 전체 ZIP 을 다시 받으면 같은 자리에서 또
    실패한다(2026-08-26 현장: 델타 실패 → 331MB 재다운로드 → 재실패).
  ...
  - **관리자 권한으로도 못 푸는 것**: 실행 중인 파일의 잠금. 강제 종료하지 않고 막힌
    경로를 안내한 뒤 기존 버전으로 실행한다.
- **기존 버전 폴더를 `rmtree` 로 지우지 않는다** (2026-08-26). 완성돼 있으면 채택(adopt),
  깨진 잔재는 `versions\<ver>.old-*` 로 **옆으로 rename** 한 뒤 best-effort 삭제한다.
  rmtree 는 잠긴 파일 하나에 실패하면서 폴더를 반쯤 파괴했고, 그 잔재가 다음 실행도
  같은 자리에서 막아 업데이트를 영구히 못 하게 만들었다. 위험한 로컬 조작은 전부
  `prepare_target()` 이 **다운로드 전에** 끝낸다.
