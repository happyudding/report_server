...
- **실행 중 Honey 종료** — 업데이트가 있을 때 같은 설치 루트의 `Honey.exe`,
  `HoneyApp.exe`, `QtWebEngineProcess.exe` 를 찾는다. 사용자에게 저장하지 않은 작업이
  사라질 수 있음을 알리고 동의받은 경우에만 창 닫기 요청 → 제한 대기 → 강제 종료 순으로
  정리한다. 거부하거나 종료하지 못하면 업데이트와 중복 실행을 모두 보류한다.
