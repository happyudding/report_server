...
- **실행 중 Honey 종료** — 같은 설치 루트의 프로세스를 실제 앱(`HoneyApp.exe`), 다른
  런처(`Honey.exe`), Qt 보조 프로세스(`QtWebEngineProcess.exe`)로 구분한다.
  - ⚠️ **런처는 PyInstaller onefile 이라 실행하면 부트로더 부모 + 실제 코드 자식,
    같은 `Honey.exe` 경로의 프로세스가 항상 2개다.** 프로세스를 세는 코드
    (`launcher.running_honey_processes`)는 `os.getpid()` 뿐 아니라 **`os.getppid()` 도
    반드시 제외**해야 한다 — 빠뜨리면 런처가 자기 부모를 "다른 런처"로 오인해 모든
    실행·업데이트가 자기 자신에게 막힌다(2026-08-26 실제 장애: 3.1.9 전원 "다른 Honey
    실행 또는 업데이트가 진행 중입니다" 차단, 재부팅 무관 100% 재현). `check_launcher.py`
    는 런처를 python 으로 돌려 이 회귀를 못 잡는다 — 빌드된 exe 로 확인할 것
    (update_test/README.md 시나리오 15). 실제 앱이
