...
| `FormulaEditor` | 신규 Item 페이지의 수식 입력 위젯(`honey_ui/formula_editor.py`). **읽기 전용 칩 스트립 + 평범한 QLineEdit + 인라인 후보 목록 + 연산자·함수 버튼**. item 이름에 공백·괄호·연산자·따옴표가 전부 합법이라 평문 렉서가 원리적으로 불가능하므로 **토큰 배열이 정본**이고 표시 문자열은 재파싱하지 않는다(웹 Gap Chart 와 같은 판단). 커밋 규칙은 모호성 0: 빈 입력 + `+ - * / ( ) , < > =` → 연산자 토큰 / 글자가 있을 때 같은 키 → 검색어 문자(`VDD-B` 같은 이름 보호) / 빈 입력 Backspace → 마지막 토큰 pop / Enter → ①후보=item ②숫자=num ③거절(**자유 텍스트 item 금지**) / `@` → 후보 전체 펼침. `>`+`=` 는 타이머 없이 `>=` 로 합친다. 함수 버튼은 `fn`+`lp` 를 함께 넣어 "괄호 없는 함수" 상태를 만들 수 없게 한다. **한글 IME 조합 중에는 키를 가로채지 않는다**(`_ImeLineEdit._composing` — 없으면 조합 확정과 동시에 엉뚱한 토큰이 들어간다). 칩 색은 항목=파란 기울임·함수=보라·비교=청록(웹 `.gc-tok-*` 와 같은 팔레트). 회귀 고정 `tests/test_new_item_dialog.py` |
