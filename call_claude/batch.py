...
def build_meta_prompt(prompts, nonce=None):
    ...
    n = len(items)
    # ⚠ 마지막 두 줄은 **바깥 배치 형식(JSON 배열)과 안쪽 요청 형식의 충돌**을 푼다
    # (2026-09-02 현장): 요청이 "JSON 으로 답하지 마라" 라고 지시하면 모델이 그 둘을
    # 헷갈려 `text` 값 **안에** 또 다른 JSON 을 넣는 일이 있었다. 배치 봉투는 이 래퍼가
    # 강제하고, `text` 값은 요청이 시킨 형식 그대로 담으면 된다는 것을 분리해 알려 준다.
    # 도메인 문구는 넣지 않는다 — 이 패키지는 재사용 대상이다(README §경계).
    head = (
        ...
        "각 text 값에는 그 요청이 시킨 형식의 답을 **문자열 하나로** 담는다 - "
        "요청이 평문을 요구하면 평문을 그대로 넣고, text 안에 또 다른 JSON 이나 객체를 만들지 마라."
    )
    blocks = []
