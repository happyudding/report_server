...
| **"AI Comment 문장이 이상하다" — 어디가 문제였나** | 같은 탭의 **흐름 디버깅 3종**(2026-09-01): 흐름 타임라인(`.../timeline` — 업로드→클라 실패→push 시간순, "워커가 아예 안 돎"까지 판정) · **LLM 이 받은 프롬프트**(`.../prompts` — 본문 전문, 재료 검증) · skip 사유 5종 분해. 둘 다 `allow_build=False`(관리자 조회가 콜드 빌드를 유발하면 안 된다). 저장 문장의 `raw` 열이 차 있으면 서버 sanitize 가 걷어낸 것 → [docs/23](docs/23_ai_comment_client_llm.md) |
| **사용자가 "AI 문장이 안 왔다"고 한다** | 클라 워커가 실행 로그·상태바에 사유를 남긴다([transport/ai_suggest.py](client/transport/ai_suggest.py) `on_progress`/`http_hint` — 403=권한/404=서버 버전/423=잠김/0=네트워크). 그 전에 Honey 의 **Claude 연결 신호등**(AI Comment 체크 옆 ●)을 눌러 실호출 1회로 인증을 먼저 가른다 |
