...
def test_output_style_rules():
    ...
    #   ("중심"은 사례를 **먼저** 적으라는 뜻이지 조치를 빼라는 뜻이 아니다 — ⑤ 참조.)
    assert "사례에서 무엇을 어떻게 확인해 해결했는지를 중심으로" in P._INSTRUCTION
    assert "그대로 옮겨 적지 마라" in P._INSTRUCTION
    # 기본 조치 목록은 재료로 계속 실린다 — [제안] 각 줄이 사례와 **함께** 담아야 한다
    assert "[기본 조치 목록(action_ko)]" in p

    # ⑤ 배포 yaml 지시문 — 프롬프트에 실제로 합류하는지까지 본다.
    #   ⚠ [제안] 한 줄의 구성은 **사례 + 기본 조치(action_ko) 결합**이다(2026-09-02 사용자
    #   확인 — "LLM 이 제안 만들 때 사례 + action_ko 로 만들어라는 의미였다"). 종전 문구는
    #   "기본 조치 목록은 사례로 덮이지 않는 signature 에만 쓰되" 라는 **배타** 관계라
    #   모델이 둘 중 하나만 골랐고, action_ko 쪽으로 기울면 사례가 통째로 사라졌다.
    #   화면 [사례] 블록은 hideCase 로 숨는 것이 정상이므로(사용자 의도), 사례가 [제안]
    #   안에 안 들어가면 사용자는 사례를 **어디서도** 볼 수 없다.
    #   yaml 은 문장을 접어 저장하므로(줄바꿈+들여쓰기) 프롬프트 문자열에서 공백을
    #   접어 비교한다 — 원문 그대로 찾으면 줄바꿈 위치에 따라 헛되이 깨진다.
    flat = " ".join(p.split())
    assert "사례 + 기본 조치를 합쳐서" in flat and "둘 중 하나만 고르는 것이 아니다" in flat, \
        "integrate_precedents 가 사례/조치 배타 선택형으로 되돌아갔다"
    assert "기본 조치를 이어 붙여라" in flat, "사례에 조치를 결합하라는 지시가 사라졌다"
    ...
    # 결합 지시는 EXTRA 에도 있어야 한다 — base _INSTRUCTION 의 배타 문장("사례로 덮이지
    # 않는 signature 를 메우는 데만 쓰고")이 프롬프트 앞쪽에 있어 뒤에서 덮어야 한다.
    # base 는 엔진 recommend.py 와 바이트 일치 사본이라 여기서 고칠 수 없다(테스트 (e)).
    assert "사례와 기본 조치를 **합쳐서** 쓴다" in P._INSTRUCTION_EXTRA, \
        "EXTRA 의 결합 지시가 사라졌다 — base 의 배타 문장이 그대로 살아난다"
    ...
    # ⑦ 금지는 **문장의 모양**에만 걸리고 사례 자체는 반드시 인용한다 (2026-09-02 개정 2차).
    #   1차 개정("생략하거나 크게 줄여라 … 아예 비워 두는 편이 낫다" 삭제)만으로는 부족했다 —
    #   사례를 **일반론으로 바꿔 쓰는** 실패가 남았다(FUNC_FAIL 선례 "강제 0xFF 써져서 Fail"
    #   → 출력 "외부요인에 의한 것인지 추가 확인 필요"). deny_patterns 는 이걸 못 잡는다
    #   (사례를 부정·변명하는 문장이 아니라 그냥 밋밋한 일반론이라 정규식의 사각지대다).
    #   그래서 **일반론 금지와 원문 인용 의무**를 지시문 4곳에 나눠 걸고 여기서 고정한다.
    assert "사례를 생략하라는 의미는 아니다" in flat, \
        "no_meta_judgment 가 다시 사례 자체를 막는 문장으로 돌아갔다"
    assert "비워 두는 편이 낫다" not in flat, \
        "omit_when_no_info 에 '빈칸이 낫다'(사례 포기 유도)가 되살아났다"
    # ⑦-a [사례] 와 [제안] 의 역할 분담 — 같은 동사쌍(판단/평가)에 정반대 지시를 주면
    #   모델은 둘 다 피하는 쪽(사례 언급 최소화)으로 도망친다. 실제 회귀의 한 축이었다.
    assert "판단·평가는 [제안] 에서 한다" in flat, \
        "summarize_only 가 다시 '판단하지 마라'로 돌아가 no_meta_judgment 와 모순된다"
    # ⑦-b 사례 원문의 구체어를 살리라는 지시 3종. 하나라도 빠지면 일반론으로 돌아간다.
    assert "일반론으로 바꾸지 마라" in flat, "cite_verdict_history 의 일반론 금지가 빠졌다"
    assert "강제 0xFF 써서 Fail 된 이력이 있음" in flat, \
        "cite_verdict_history 의 원문 인용 예시(0xFF)가 빠졌다 — 모델이 형태를 모른다"
    assert "가성으로 판단된 이력이 있음" in flat, \
        "cite_verdict_history 의 한마디 사례 예시가 빠졌다"
    assert "PGM 확인 및 time 최적화 검토 필요" in flat, \
        "cite_verdict_history 의 구체 조치 사례 예시가 빠졌다"
    # ⑦-c 사례 속 코드값(0xFF·bin 번호)은 **내부 지표명 금지의 대상이 아니다**.
    #   FUNC_FAIL 처럼 값이 판정 코드인 item 은 이 예외가 없으면 핵심 어휘가 통째로 회피된다.
    assert "0xFF 등" in flat, "no_metric_names 의 사례 코드값 예외가 빠졌다"
    # ⑦-d 사례 내용은 terse_lines 가 말하는 '군더더기'가 아니다(짧게 쓰되 빼지는 말 것).
    assert "군더더기가 아니라 핵심이다" in flat, \
        "terse_lines 가 사례 원문까지 깎는 무조건형으로 돌아갔다"
    # ⑦-e 사례가 있는데 "추가 확인이 필요하다" 로 때우는 문형을 지시문이 직접 막는다.
    assert "일반론으로 때우지 마라" in flat, \
        "signature_budget_first 가 다시 '추가 확인' 문형을 유도한다"
    # ⑦-f _INSTRUCTION_EXTRA — "결과로 무엇을 확인할지만 남겨라" 는 사례 내용을 빼고 액션만
    #   남기라는 지시로 읽혀 바로 그 일반론을 유도했다. 되돌림 방지.
    assert "결과로 무엇을 확인할지만 남겨라" not in P._INSTRUCTION_EXTRA, \
        "EXTRA 에 사례 내용을 버리게 하는 옛 문장이 되살아났다"
    assert "그대로 살려 써라" in P._INSTRUCTION_EXTRA, \
        "EXTRA 의 사례 내용 보존 지시가 사라졌다"
