...
| `GET` | `/distribution_batch?subjects=…&order=seq` | 공개 | 같은 라우트의 **Serial 순**(rawdata 누적 순) 변형 — ECDF 대신 **행 순서를 보존한 값 배열**(포맷 `seq-columnar-v1`). Distribution "Serial 순" 토글 전용. `order` 는 화이트리스트(`""`/`ecdf`/`seq`, 그 외 400)이고 캐시·ETag 가 ECDF 와 갈린다 |
...
| `GET` | `/api/my_messages` | 공개 | **관리자 팝업 메시지** 수신 — 아직 확인하지 않은 것만. 세 페이지(랜딩·검색결과·세션 상세)의 `admin_message.js` 가 30초 폴링(화면이 보일 때만). 수신자 키는 사용량 집계와 같은 규칙(신원 있으면 소문자 계정, 없으면 `ip:<addr>`)이라 **신원 없는 브라우저는 전체 공지만** 받는다. 저장소는 서버 프로세스 메모리(`admin_panel/messages.py`) — 재시작 시 미확인분 소멸. 이 폴링은 접속현황 하트비트도 겸해서 `?page=`(`index`/`view`/`landing`)·`?sid=`·`?idle=`(마지막 입력 이후 초)를 함께 싣는다 — **표시 전용이며 위조 가능**하므로 접근제어·감사에는 쓰지 않는다(없어도 응답은 동일) |
...
**접속 유지(`ago`)와 실제 행동(`input_ago`)은 다른 값이다.** 리포트 페이지는 30초마다
공지를 확인하므로, 마지막 요청 기준으로 보면 화면을 켜둔 채 자리를 비운 사람도 늘 활동
중으로 보인다. 그래서 그런 폴링(`my_messages`·`build_status`, 그리고 실사용과 라우트를
공유하는 폴링이 붙이는 `?hb=1`)은 **접속 유지로만 세고** 마지막 경로·보는 세션·활동
타임라인을 덮지 않는다. `input_ago` 는 실사용 요청과 하트비트의 `idle` 힌트로만 갱신되며,
관리자 화면의 초록(60초 이내 입력)/노랑(접속은 유지, 입력 없음)이 이 값으로 갈린다.
힌트를 안 보내는 옛 클라는 `input_ago`=`null` 이고 화면이 종전 `ago` 기준으로 폴백한다.
`page` 는 하트비트가 알려준 화면 종류(`index`/`view`/`landing`)로, 조용한 사용자가 어디에
있는지 보여 준다. Honey 앱은 하트비트가 없지만 모든 요청이 실사용이라 그대로 초록이 된다.
