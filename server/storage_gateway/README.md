...
| 함수 | 용도 |
|------|------|
| `save_upload_artifacts(*, analysis_key, content_hash, meta_str, xlsx_bytes, issue_images=None, dist_png=None, chart_pngs=None)` | 업로드 1건의 모든 산출물 저장. `{s3_ok, warnings, issue_images_saved, distribution_combined, charts_saved}` 반환 |
...
yield_text/<akey>.json           issue_table_text/<akey>.json
...
- `analysis_key` = `sha256(xlsx_bytes + canonical(meta))`. 저장 경로/키는 바꿔도
